...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch('./json/animeUpdate.json')
    ...
    .then(movies => {
      ...
      allMovies = movies.sort((a, b) => b.score - a.score);
      loadMovies(); // Tampilkan 20 film pertama
    })
    .catch(error => {
      console.error('Error fetching movie data:', error);
    });

  // Fungsi untuk memuat 20 movie pada setiap pemanggilan
  function loadMovies() {
    const moviesListElement = document.getElementById('movies-list1');
  ...
  document.getElementById('load-more1').addEventListener('click', loadMovies);
